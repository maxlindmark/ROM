...
This is not a very straightforward plot to make. First you need to manually find a value that scales the differnt units so they become close (see above when making p3f). What's more of a challenge however is that we no longer have a full legend. This is because in all previous plots, the legend is inheretided from the aes, where we set color = group. We need to exclude the group here when first making the plots, because theNote we also did not include errorbars.
