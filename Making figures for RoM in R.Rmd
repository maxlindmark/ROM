...
dat1 <- dat %>% 
  select(År, Response_variable, Group) %>% 
...
p1 <- ggplot(dat1, aes(År, Response_variable)) +
  geom_bar(data = dat1, 
           aes(x = År, y = Response_variable), stat = "identity", color = pal[1], fill = pal[1], 
           width = 0.6) +
...
p2 <- ggplot(dat, aes(År, Response_variable, color = Group)) +
  geom_bar(data = subset(dat, Group == main_series), 
           aes(x = År, y = Response_variable), stat = "identity", color = pal[1], fill = pal[1], 
           width = 0.6) +
  geom_line(data = dat, aes(År, Response_variable, color = Group, alpha = Group), 
            size = 1) + 
  geom_point(data = subset(dat, Group == special_series), # here we set our special series
             aes(År, Response_variable, fill = Group), size = 2, color = pal[max(n_lev)]) + 
...
p2a <- ggplot(dat2, aes(År, Response_variable, color = Group)) +
  geom_bar(data = subset(dat2, Group == main_series), 
           aes(x = År, y = Response_variable), stat = "identity", color = pal[1], fill = pal[1], 
           width = 0.6) +
  geom_line(data = dat2, aes(År, Response_variable, color = Group, alpha = Group), 
            size = 1) + 
